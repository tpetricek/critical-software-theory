...
At first sight, the formal structures designed by Peter Eisenman seem similar to modernist
architecture (Figure~\ref{fig:wtc}) and some of his critics saw his focus on formalism and
abstraction as a return to modernist ideals. As I illustrated through a number of examples,
Eisenman works with simple formal modernist structures, but he uses them for a different
purpose than modernist architects. His project for the World Trade Center memorial, for
example, turns the horizontal Manhattan grid into a vertical one.
...
Those formal models are based on existing programming patterns. For example, Hoare explicitly notes
...
structures that describe some aspect of computation or program structure, but are not
directly derived from practical programming languages. An interesting question to ask is how
those structures resolve the problem of achieving fit between the form and context. Does the
fit emerge as a (perhaps) accidental effect of the use of formal structures? Or does the use
of autonomous formal structures result in ``unlivable'' buildings, as critics of Eisenman
occasionally point out?

If we see the lambda calculus as the formal structure that underpins functional programming
languages, then there are certainly many areas where the emergent form fits with a context.
There are many areas where functional programming languages have been successful and highly
influential. Functional list processing, which inspired LINQ and Java Streas would be only
one such example. This claim necessitates some caution, though. The Lisp language, often regarded
...
example of functional list processing also does not require nearly the full power of the
lambda calculus.\endnote{See \url{https://programming-journal.org/2020/4/8/}}
...
\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{fig/upsidedown.jpg}
\caption{Upside down.}
...
The example in Figure~\ref{fig:monads}
uses monads and the \texttt{do} notation to


~

important to note that formal structures do not automatically imply good fit!
